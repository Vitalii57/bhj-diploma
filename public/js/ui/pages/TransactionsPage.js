...
class TransactionsPage {
  ...
  formatDate(date) {
    ...
    return (stringDateTransaction = `${dateN.getDate()} ${
      monthString[dateN.getMonth()]
    }  ${dateN.getFullYear()} г. в ${dateN.toISOString().slice(11, 16)}`);
  }
  ...
  renderTransactions(data) {
    if (data) {
      ...
      data.forEach((elem) => {
        addListTransactions.insertAdjacentHTML(
          "beforeend",
          this.getTransactionHTML(elem)
        );
      });
    }
  }
